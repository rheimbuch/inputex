(function() {
   ...
   // Create inputEx.widget namespace if it doesn't exist
   if(YAHOO.lang.isUndefined(inputEx.widget)) {
      inputEx.widget = {};
   }
    ...
    * Date Extensions:
    */
   Date.getToday = function() { return (new Date()).getMidnight(); };
   ...
   Date.prototype.AddWeeks = function(amount) { return this.AddDays(7*amount); };
...
inputEx.widget.calendar.prototype = {
   ...
   setOptions : function(options) {
      ...
      this.cfgPageDate = (config.pageDate) ? config.pageDate.findMonthStart() : (new Date()).findMonthStart(); // reference date for displaying cals (if = "13 november 2007" and calNumber = 2, then save "1st november 2007" and  display 2 cals : november 2007 and december 2007) 
      ...
      this.lastSelection = {};
      this.lastSelection.date = config.selectedDate || Date.getToday();
   },
 ...
 */
   ...
   updateMonthHeader : function(cal_position) {
      ...
      var workingDate = this.cfgPageDate.AddMonths(cal_position);
...
updateBody : function(cal_position) {
   ...
   var calDate = this.cfgPageDate.AddMonths(cal_position);
   var startMonthDate = calDate.findMonthStart();
   var endMonthDate = calDate.findMonthEnd();
   ...
   var startCalDate = startMonthDate.AddDays(1-startMonthDayNumber);
   ...
   for (i=0;i<this.cfgCalNbrLines;i++) {
      ...
      for (j=0;j<7;j++) {
         td_cell = cells[j];
         cell_index = i*7+j;
         cell_date = startCalDate.AddDays(cell_index);
...
markSelectedCell : function() {
   ...
   if (this.lastSelection && this.lastSelection.date) {
      ...
      var endLastMonthDate = startFirstCalDate.AddMonths(this.cfgCalNumber).findMonthEnd();
      ...
      for (i=0; i<tbodies.length; i++) {
        ...
        calDate = this.cfgPageDate.AddMonths(i); // i = cal_position
        startMonthDate = calDate.findMonthStart();
        endMonthDate = calDate.findMonthEnd();
        ...
        // startCalDate : date of the first cell of the body (not necessarily first date of month).
        startCalDate = startMonthDate.AddDays(1-startMonthDayNumber);
        ...
        for (j=0; j<this.cfgCalNbrLines; j++) {
           ...
           for (k=0; k<7; k++) {
              td = cells[k];
              cellDate = startCalDate.AddDays(j*7+k);
...
updateLastSelection : function(selectedCellId) {
   ...
   // Hack when we are on 31 of august for instance : findMonthStart executed before adding month 
   var calDate = this.cfgPageDate.AddMonths(calPosition);
   var startMonthDate = calDate.findMonthStart(calDate);
   ...
   var cellDate = new Date((startCalDate.AddDays(cellLine*7+cellColumn)).getTime());
...
updateSelectedDates : function() {
   ...
   addMonths : function(count) {
      this.cfgPageDate = this.cfgPageDate.AddMonths(count);
   },
...
nextDates : function(offset) {
   ...
   var currentDate = this.lastSelection.date.AddDays(1);
   var currentDay;
   var counter = 0;
   ...
   while (counter<7*dayOffset) {
      ...
      if (counterOffset===1) {
         ...
         var lastCalDate = this.cfgPageDate.AddMonths(this.cfgCalNumber-1);
         var endMonthDate = lastCalDate.findMonthEnd(lastCalDate);
...
previousDates : function(offset) {
   ...
   gotoDate : function(date) {
      this.cfgPageDate = date.findMonthStart();
